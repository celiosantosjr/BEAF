	...
	case $T2 in 
		...
		I|i)
			...
			then
				...
				cp $R1 $address
				echo "# We will convert merged file to fasta format."
				gunzip -c *.gz | awk '{if(NR%4==1) {printf(">%s\n",substr($0,2));} else if(NR%4==2) print;}' > FastaQ-zcat.fa
				echo "# Removing unwanted files..."
				rm -rf *.gz
			fi
		;;
				...
				for sub in `cat list`; do
					...
					case $T1 in
						...
						N|n)
							...
							#megablast -d $sub -i hits -e 1e-5 -m 8 -o $sub.tmp -v 50 -b 0 -a 8 -W 50 -z 0 -S 3
							dsp2=`date -u "+%s"`
							echo $dsp2 - $dsp1 |bc -l > $sub.ft.time2
						;;
					esac
...
print \"the Subset fasta file\", subsetName, \"is now created\"" > ext.py
				...
				for File in `cat list`; do
					...
					then
						...
						python spades.py -k 11,15,21,25,31,35,41,45,51 --only-assembler -s $File.fasta -o assembly_$Out_$File
					...
					rm -rf *.time2
					cd $address/OUTPUT/$Out
					echo "${File/.f*/}|$sq|$ppm2|$cntg|$BTime|$STime|$TTime|$Warnings" > tmp1.$File
				done
				cd $address/OUTPUT/$Out
				rm -rf *.time2
				mv hits hits.fasta
				...
				rm -rf $address/spades/bin/assembly_*
				echo "-----------------------------------------------------------------------------
Subref_database|Hits_seq.|Ppm|Contigs|Blast_Time|SPADES_Time|Total_SubRef_Time|Status" > log.header
				sed -i 's/|/\t/g' log.header
				cat log.header tmp1.* > log.tmp
				sed -i 's/|/\t/g' log.tmp1
